...
namespace LinkQuery
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var Garments = from p in Product
                           join c in ProductCategory
                           on p.ProductModelID equals c.ProductCategoryID
                           join m in Product
                           on p.ProductID equals m.ProductCategoryID
                           select new { ProductID = p.ProductID, Name = p.Name, Color = p.Color, StandardCost = p.StandardCost, ListPrice = p.ListPrice, Size = p.Size, Weight = p.Weight, ProductCategoryID = p.ProductCategoryID, ProductModelID = p.ProductModelID };
            foreach (var g in Garments)
            {
                Console.WriteLine($"{g.Name}\t{g.Color}\t{g.StandardCost}\t{g.ListPrice} ");
